...
# Jab user card iconme ya image per click karen to function run hoga
def showCart(request, pk):
    cart = request.session.get('cart', [])
    all_Add_Items = []
    total_real_price = 0
    total_previous_price = 0
    total_discount = 0
    ...
    for i in cart:
        ...
        total_real_price += item.db_product_real_price
        total_previous_price += item.db_product_previous_price
        total_platform_fee += 4

    total_discount = total_previous_price - total_real_price
    total_ammount = total_real_price + total_platform_fee

    # ✅ Apply delivery charge only if total >= 2000
    if total_ammount >= 2000:
        total_delivery_charges = 29
        total_ammount += total_delivery_charges

    total_save_ammount = total_previous_price - total_ammount
    userdata = Customer.objects.get(id=pk)

    return render(request, 'addToCart.html', {
        'all_Add_Items': all_Add_Items,
        'userdata': userdata,
        'total_First_amount': total_previous_price,
        'total_item': len(all_Add_Items),
        'total_discount': int(total_discount),
        'total_ammount': int(total_ammount),
        'total_platformFee': int(total_platform_fee),
        'total_Delivery_Charges': int(total_delivery_charges),
        'total_save_ammount': int(total_save_ammount),
    })
...
def goForPayment(request,pk,cd):
    ...
    count = 1
    price = 0
    price = cardData.db_product_real_price + 4 + 29
    # Sirf id store karo session me
    request.session['card_id'] = cardData.id
    return render(request,'goForPayment.html',{'userdata':userdata,'msg':'one','cardData':cardData,'count':count,'price':price,})
...
def buyAllAddToCartItem(request,pk):
    ...
    for i in cart:
        ...
        delivery = delivery + 4
    if price > 1000:
        totalAmmount = price + delivery + 29
    else :
...
# Final Payment ke Liye :
def payment(request, pk):
    if request.method == "POST":
        userdata = Customer.objects.get(id=pk)
        cart = request.session.get('cart', [])
        card_id = request.session.get('card_id')  # Single product purchase ID
        msg = 'All Data'
        all_data = []
        total_item_price = 0
        count = 0
        cardData = None  # For single item case
        delivery = 0

        if card_id:
            # Single product purchase
            try:
                cardData = DynamicCards.objects.get(id=card_id)
                total_item_price = cardData.db_product_real_price
                msg = 'one'
                count = 1
                # Remove card_id after use
            ...
            for i in cart:
                try:
                    data = DynamicCards.objects.get(id=i)
                    all_data.append(data)
                    total_item_price += data.db_product_real_price
                    delivery = delivery + 4
                except DynamicCards.DoesNotExist:
                    continue
            msg = 'All Data'
        ...
        gst = 29 if total_item_price > 1000 else 0
        total_price = total_item_price + delivery + gst
        amount = total_price * 100  # Convert to paisa for Razorpay
        ...
        # Save order
        ProductOrderDetailAfterPayment.objects.create(amount=amount,
                                                      order_id=payment['id'],
                                                       )

        return render(request, "goForPayment.html", {
            'all_data': all_data,
            'totalAmmount': total_price,
            'payment': payment,
            'userdata': userdata,
            'amount': amount,
            'msg': msg,
            'count': count,
            'cardData': cardData  # Only used if msg == 'one'
        })
...
def payment_status(request):
       if request.method=="POST": 
        ...
        try:
            ...
            product = ProductOrderDetailAfterPayment.objects.get(order_id=response['razorpay_order_id'])
